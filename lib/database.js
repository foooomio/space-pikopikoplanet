...
export const fetchUserDataByUserId = (userId) => {
  return db
    .collection('users')
    .where('userId', '==', userId)
    .get()
...
const saveUserDataInUsers = (user) => {
  return db.collection('users').doc(user.uid).set(user);
};

const updateUserDataInQueries = (user) => {
...
export const saveUserData = (user) => {
  return Promise.all([
    saveUserDataInUsers(user),
    updateUserDataInQueries(user),
  ]);
};
...
export const fetchQueryList = (cursor, limit, searchOptions = {}) => {
  let query = db
    .collection('queries')
...
export const fetchQueryListLikedByUser = (uid, cursor, limit) => {
  return db
    .collection('users')
    .doc(uid)
    .collection('likes')
...
export const fetchEndpointList = () => {
  return db
    .collection('queries')
    .get()
    .then((querySnapshot) => {
